...
Multi-channel notifications for Strav 1.0. One fluent surface (`notifications.send(notifiable, notification)`) that fan-outs to ≥1 channel drivers — mail / database / log / webhook today; broadcast / Discord / SMS channels in follow-up slices.
...
Deferred: broadcast, Discord, SMS channel drivers. Apps register custom channels via `manager.extend(name, factory)`.
