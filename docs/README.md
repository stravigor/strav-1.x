...
| `@strav/view` | M3 — engine core shipped (tokenizer + compiler + `ViewEngine` + `ViewProvider` + frozen directive set minus `@island`). `@island` + Vue bundler + client runtime + pages auto-router still to come. See `docs/view/` |
