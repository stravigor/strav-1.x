...
> **Status: 1.0.0-alpha.3 — M2 shipped + post-M2 polish.** Foundation + ORM + DDL + diff (additive + destructive + type/nullability drift via `allowAlter`) + tenancy + lifecycle events + unit-of-work + soft-delete + relations + decorators (`@hidden` / `@cast` / `@ulid` / `@encrypt`) + advisory locks + per-tenant sequencing + two-role connection config + boot-time tenant-registry validation + encryption-at-rest all on npm as `@strav/database@1.0.0-alpha.3`.
