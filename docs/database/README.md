...
> **Status: 1.0.0-alpha — M2 in progress (foundation + ORM + DDL + diff + tenancy + lifecycle hooks + unit-of-work + soft-delete + relations + decorators).**
...
> Deferred (each is its own slice): **repository lifecycle hooks** (`<resource>.creating` / `.created` / etc. on the EventBus), **soft-delete integration** (`.withTrashed()`, `delete()` writing `deleted_at`), **relationships + eager loading**, **pagination helpers**, **joins + CTEs**, **destructive diff** (dropped tables/columns, type changes, renames — needs explicit `--allow-drop` semantics), **migration builder DSL** (`m.createTable(name, fn)` / `m.addIndex(...)` etc.), **two-role connection config** (app vs BYPASSRLS roles — apps wire manually today), **`tenantedBigSerial` per-tenant sequencing** (today emits `bigint`; trigger + sequence + composite PK deferred), **tenancy diff awareness** (the diff generator doesn't auto-add tenant_id/RLS to existing tenanted tables), **Repository<TModel> auto-routing inside `withTenant`**, the **`db:migrate` / `db:rollback` / `db:status` / `make:migration` console commands** (need `@strav/cli` integration).
...
- [`guides/relationships.md`](./guides/relationships.md) — `t.hasMany` / `t.belongsTo` declarations, `.with(...)` eager loading (one batched SELECT per relation, N+1 prevention), offset pagination via `.paginate({ page, perPage })`, what's deferred (typed children, nested loads, cursor pagination, lazy loading).
