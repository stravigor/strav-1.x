...
> **Status: 1.0.0-alpha.2 — M2 shipped.** Foundation + ORM + DDL + diff (additive + destructive) + tenancy + lifecycle events + unit-of-work + soft-delete + relations + decorators (`@hidden` / `@cast` / `@ulid` / `@encrypt`) + advisory locks + per-tenant sequencing + two-role connection config + boot-time tenant-registry validation + encryption-at-rest all on npm as `@strav/database@1.0.0-alpha.2`.
