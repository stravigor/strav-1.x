...
The AI module for Strav 1.0 — a unified `Provider` interface, a per-app `BrainManager` facade, multi-turn `Thread`s, and built-in prompt caching. V1 ships **four** providers: **Anthropic** (`@anthropic-ai/sdk`), **OpenAI** (`openai`), **Gemini** (`@google/genai`), and **DeepSeek** (subclass of OpenAI pointed at DeepSeek's compat endpoint).
