...
| `AnthropicBrainDriver` | Concrete `Provider` wrapping `@anthropic-ai/sdk`. Translates framework shapes ↔ SDK shapes |
| `OpenAIBrainDriver` / `OpenAIProviderConfig` | Concrete `Provider` wrapping `openai`. `chat / stream / runWithTools` mapped to chat-completions; `countTokens` not implemented; server-side MCP rejected (use Anthropic) |
...
- [`guides/openai.md`](./guides/openai.md) — `OpenAIBrainDriver` config, shape translation (system prompts, tool definitions, tool-result fan-out), reasoning-effort mapping, what's not supported (`countTokens`, server-side MCP), tier remapping for OpenAI apps.
- [`guides/openai-responses.md`](./guides/openai-responses.md) — `OpenAIResponsesBrainDriver` (driver `'openai-responses'`) for OpenAI's Responses API. Unlocks `web_search` + `code_interpreter` server tools. Inherits `embed` + `transcribe` from `OpenAIBrainDriver`. Pair with `'openai'` and route per-call.
- [`guides/gemini.md`](./guides/gemini.md) — `GeminiBrainDriver` config, shape translation (`assistant`→`model`, `systemInstruction`, `functionDeclarations` / `functionCall` / `functionResponse`), `thinkingConfig` mapping, MCP via the local client, tier remapping for Gemini apps.
- [`guides/deepseek.md`](./guides/deepseek.md) — `DeepSeekBrainDriver` config; inherits OpenAI's wire format with three deltas (no `reasoning_effort`, no `response_format.json_schema`, DeepSeek-specific cache field); combined tools+schema deferred; recommended subclassing pattern for other OpenAI-compatible vendors (Groq, Together, …).
- [`guides/ollama.md`](./guides/ollama.md) — `OllamaBrainDriver` for local + open-weights models (Llama 3.2 / Qwen 2.5 / Mistral / …). Privacy-preserving + free dev runs. Works against Ollama, LM Studio, llama.cpp's server, vLLM by overriding `baseUrl`.
