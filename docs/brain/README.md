...
- [`guides/multimodal.md`](./guides/multimodal.md) — Images, PDFs, audio. `ImageBlock` works on all five providers (model-dependent on local). `DocumentBlock` (PDF) native on Anthropic + Gemini; OpenAI / DeepSeek / Ollama throw with split-to-images guidance. `AudioBlock` native on Gemini; everyone else throws with provider-specific guidance.
