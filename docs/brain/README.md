...
- [`guides/multimodal.md`](./guides/multimodal.md) — `ImageBlock` for vision-capable models. `{ type: 'image', source: base64 | url }` translates to each provider's native shape (Anthropic image block / OpenAI image_url / Gemini inlineData|fileData). Vision-model picks per provider; local + private path via Ollama vision models.
